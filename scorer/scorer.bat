...
my ($metric, $keys_file, $response_file) = @ARGV;
print "\n=======================================================\nFile: $keys_file\n";
...
if ($metric eq 'all') {
  foreach my $m ('muc', 'bcub', 'ceafm', 'ceafe') {
    $CorScorer::VERBOSE = 2;
    print "\nMETRIC $m:\n";
    my @actual_counts = &CorScorer::Score( $m, @ARGV );
    my ($numrec, $denrec, $numpre, $denpre) = @actual_counts;

    &CorScorer::ShowRPF( $numrec, $denrec, $numpre, $denpre );
  }
}
else {
  print &CorScorer::Score( $metric, @ARGV );
}
};
